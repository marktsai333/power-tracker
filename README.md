# power-tracker

電費追蹤 PWA。輸入電表累計讀數，自動換算計費週期內的用電量、費用與趨勢。

**Live:** https://marktsai333.github.io/power-tracker/

單一 HTML 檔，無相依套件、無建置流程。資料儲存於瀏覽器 `localStorage`，不經過任何伺服器。

## 功能

- 首期從第一筆初始電表讀數起算，到下一個結算日結束；之後以結算日（預設每月 15 號）切期，跨週期的抄表區間按時間比例分攤
- 本期預估帳單：已抄錄區間為實算，剩餘天數以近 30 天平均速率推估
- 用電量趨勢圖，折線 / 柱狀可切換，含平均基準線
- 圖表可左右滑動瀏覽歷史資料；離開最新資料時出現「回到今天」。切到日／週／月會分別聚焦最近 14 天／12 週／12 個月。
- 橫軸切成整日 / 整週 / 整月，抄表區間會按時間分攤到跨到的每一格，抄得密或抄得疏都能互相比較
- 顏色分辨資料來源：實際量到的、由抄表區間分攤的、資料還不完整的，各有不同深淺
- 還沒抄表的期間（例如今天剩下的時間）用最近的日均推估，畫成虛線框，不會混進實測值
- 時間單位（日 / 週 / 月）依資料跨度與畫面寬度自動挑選，也可以手動切換
- 歷史帳單：各期用量、費用與環比變化；可收合，預設展開
- 抄表紀錄可收合，預設收起
- 抄表紀錄可記到分鐘，同一天可抄多次；可就地修改日期、時間與度數，或刪除
- 電價、結算日可設定
- JSON 匯出 / 匯入，CSV 匯出
- 深淺色主題、離線可用、iOS 啟動畫面、應用程式內版本更新

## 安裝（iOS）

以 Safari 開啟 Live 網址 → 分享 → 加入主畫面。之後以獨立視窗啟動，離線可用。

## 專案結構

| 檔案 | 說明 |
| --- | --- |
| `index.html` | 應用程式全部內容（markup、style、logic） |
| `sw.js` | Service worker，cache-first 搭配背景更新 |
| `chart-nav.js` | 圖表歷史日期範圍與觸控操作；不修改抄表資料 |
| `manifest.webmanifest` | PWA manifest |
| `icon-*.png`、`splash/` | 圖示與 iOS 啟動畫面，由指令碼產生 |
| `make_icons.py` | 產生應用程式圖示 |
| `make_splash.py` | 產生啟動畫面並輸出對應的 `<link>` 標籤，需要 Pillow |
| `HACKING.md` | 實作說明與修改指引 |

## 資料格式

```json
{
  "price": 6,
  "settleDay": 15,
  "chartMode": "line",
  "chartUnit": "auto",
  "readings": [
    { "d": "2026-08-15T08:30", "k": 5429.9 },
    { "d": "2026-08-15T19:10", "k": 5436.2 }
  ]
}
```

`d` 為抄表時間（`YYYY-MM-DDTHH:MM`，亦接受不含時間的 `YYYY-MM-DD`，視同當日 00:00），`k` 為電表累計讀數（度）。`d` 同時是記錄的識別鍵，因此同一天可有多筆不同時間的紀錄。儲存於 `localStorage` 鍵值 `powerTracker.v1`，匯出格式與此相同。

## 計算方式

電表讀數為累計值，所有指標皆由相鄰兩筆讀數的差推導：

```
intervals()    N 筆讀數 → N-1 段區間，各含 days / used / perDay
windowUsage()  任意時間窗內的用量，跨越邊界的區間依時間比例分攤
cycles()       依結算日切出各計費週期，套用 windowUsage() 求該期用量
```

用量按時間比例分攤，因此抄表日不需對齊結算日。首期從第一筆初始讀數開始，後續週期才固定由結算日切分。讀數低於前一筆（換表或輸入錯誤）的區間會被標記並排除於統計之外；資料未完整涵蓋的週期不計入歷史帳單。

## 開發

```bash
python3 -m http.server 8731
```

## 發佈

推送至 `main` 後由 GitHub Pages 自動部署。**每次發版須同步更新兩處版本號**：

- `index.html` 的 `VERSION`
- `sw.js` 的 `CACHE`

Service worker 以 cache 名稱判斷版本，未更新則用戶端不會取得新版。應用程式內「設定與資料 → 檢查更新」可立即拉取並重新載入。
